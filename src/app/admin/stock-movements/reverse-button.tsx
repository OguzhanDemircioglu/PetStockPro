...
export function ReverseButton({ movementId, withinWindow, isTransfer }: Props) {
  ...
  if (errMsg) {
    return (
      <span
        className="rounded bg-danger-soft px-1.5 py-0.5 text-[11.5px] font-bold text-danger-7"
        title={errMsg}
      >
        ✕ {errMsg.slice(0, 30)}
      </span>
  ...
  return (
    <button
      ...
      className="rounded border border-line bg-paper px-2 py-0.5 text-[11.5px] font-bold text-ink-2 hover:border-cat hover:text-cart disabled:opacity-50"
    >
      {pending ? '...' : '↶ Geri al'}
    </button>
  );

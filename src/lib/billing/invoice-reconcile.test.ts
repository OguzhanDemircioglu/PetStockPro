...
describe('runInvoiceReconcile', () => {
  ...
  it('pending fatura → Nilvera başarılı → issued (externalRef idempotent)', async () => {
    ...
    expect(nilvera.createInvoice).toHaveBeenCalledWith(expect.objectContaining({ externalRef: 'inv-1' }));
  ...
  it('Nilvera fail → retry++ + lastNilveraError; MAX denemede alert', async () => {
    ...
    const nilvera = { createInvoice: vi.fn().mockRejectedValue(new Error('boom')) };
    const s = await runInvoiceReconcile({ db, nilvera, now });
  ...
  it('VKN yok → skip (Nilvera çağrılmaz) + retry++', async () => {
    ...
    expect(calls.updates[0].lastNilveraError).toContain('VKN');
  });
});

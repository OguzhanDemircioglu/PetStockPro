...
// ─────────────────────────────────────────────────────────────────
// İlk 100 Promo — T-7, T-1 hatırlatma + T+0 bitiş bildirimi (2026-05-22)
// ─────────────────────────────────────────────────────────────────

export interface PromoReminderTemplateInput {
  ownerName: string | null;
  companyName: string;
  daysRemaining: number;  // 7 veya 1
  promoUntil: Date;
  upgradeUrl: string;     // /admin/settings → Plan ve Fatura sayfası
  productCount: number;   // Mevcut ürün sayısı (50+ ise farklı vurgu)
}

export function buildPromoReminderTemplate(input: PromoReminderTemplateInput): VerifyEmailTemplate {
  const greeting = input.ownerName ? `Merhaba ${input.ownerName},` : `Merhaba ${input.companyName},`;
  const untilTr = input.promoUntil.toLocaleString('tr-TR', {
    dateStyle: 'long',
    timeStyle: 'short',
    timeZone: 'Europe/Istanbul',
  });
  const isLast = input.daysRemaining === 1;
  const overLimit = input.productCount > 50;

  const html = emailShell(`
    <h2 style="margin:0 0 16px;font-size:22px;color:${BRAND_CART};letter-spacing:-.3px;">
      ${isLast ? '⏰ PRO promosyonun yarın bitiyor' : `🎉 PRO promosyonun ${input.daysRemaining} gün sonra bitiyor`}
    </h2>
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;">${greeting}</p>
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;">
      İlk 100 promosyonu kapsamında 3 ay boyunca ücretsiz aldığın PRO planı
      <strong>${untilTr}</strong> tarihinde sona eriyor.
    </p>
    ${overLimit ? `
    <div style="border-left:4px solid ${BRAND_CAT};padding:12px 16px;background:#fff5ef;border-radius:0 8px 8px 0;margin:0 0 24px;">
      <strong style="display:block;font-size:13px;color:${BRAND_CART};margin-bottom:6px;">
        ⚠ Şu an ${input.productCount} ürünün var
      </strong>
      <span style="font-size:13px;color:#5f6b7c;line-height:1.6;">
        Promosyon bittikten sonra FREE plan'da 50 ürün limitin olacak.
        ${input.productCount} ürünün korunur ama <strong>yeni ürün ekleyemezsin</strong>.
        Aboneliği başlatırsan ürünlerini sınırsızca yönetmeye devam edersin.
      </span>
    </div>
    ` : `
    <p style="margin:0 0 24px;font-size:14px;line-height:1.6;color:#5f6b7c;">
      ${input.productCount} ürününle FREE plan limitinin (50) altındasın. Promosyon
      bittikten sonra istersen FREE'de devam edebilir, istersen PRO'ya geçebilirsin.
    </p>
    `}
    <div style="text-align:center;margin:0 0 24px;">
      <a href="${input.upgradeUrl}" style="display:inline-block;padding:14px 28px;background:${BRAND_CART};color:#fff;font-weight:bold;text-decoration:none;border-radius:8px;font-size:14px;">
        PRO Aboneliğini Başlat (1.000₺/ay)
      </a>
    </div>
    <p style="margin:16px 0 0;font-size:11px;color:#94a0b0;line-height:1.5;">
      Bu hatırlatma promosyonun bitiş tarihinden ${input.daysRemaining} gün önce gönderilir.
    </p>
  `);

  const text = `${greeting}

PRO promosyonun ${untilTr} tarihinde sona eriyor (${input.daysRemaining} gün kaldı).

${overLimit
    ? `⚠ Şu an ${input.productCount} ürünün var. Promosyon bittikten sonra FREE plan'da 50 ürün limitin olacak. Mevcut ürünlerin korunur ama yeni ürün ekleyemezsin.`
    : `Şu an ${input.productCount} ürünün var (FREE 50 limit'in altında). İstersen FREE'de devam edebilir, istersen PRO'ya geçebilirsin.`}

PRO Aboneliği Başlat: ${input.upgradeUrl}
...
© 2026 PetStockPro`;
  ...
  return {
    subject: isLast
      ? '⏰ PetStockPro · PRO promosyonun yarın bitiyor'
      : `PetStockPro · PRO promosyonun ${input.daysRemaining} gün sonra bitiyor`,
    htmlContent: html,
    textContent: text,
  };
}

export interface PromoExpiredTemplateInput {
  ownerName: string | null;
  companyName: string;
  productCount: number;
  upgradeUrl: string;
...
export function buildPromoExpiredTemplate(input: PromoExpiredTemplateInput): VerifyEmailTemplate {
  const greeting = input.ownerName ? `Merhaba ${input.ownerName},` : `Merhaba ${input.companyName},`;
  const overLimit = input.productCount > 50;
  ...
  const html = emailShell(`
    ...
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;">${greeting}</p>
    <p style="margin:0 0 16px;font-size:14px;line-height:1.6;">
      İlk 100 promosyonu kapsamında 3 ay boyunca aldığın PRO planın bugün sona erdi.
      Hesabın artık FREE plan'da.
    </p>
    ${overLimit ? `
    <div style="border-left:4px solid ${BRAND_CAT};padding:12px 16px;background:#fff5ef;border-radius:0 8px 8px 0;margin:0 0 24px;">
      <strong style="display:block;font-size:13px;color:${BRAND_CART};margin-bottom:6px;">
        ⚠ ${input.productCount} ürünün FREE limitinin (50) üzerinde
      </strong>
      <span style="font-size:13px;color:#5f6b7c;line-height:1.6;">
        Mevcut ürünlerin korunmaktadır ama <strong>yeni ürün ekleyemezsin</strong>.
        Devam etmek için PRO aboneliği başlat.
      </span>
    </div>
    ` : ''}
    <div style="text-align:center;margin:0 0 24px;">
      <a href="${input.upgradeUrl}" style="display:inline-block;padding:14px 28px;background:${BRAND_CART};color:#fff;font-weight:bold;text-decoration:none;border-radius:8px;font-size:14px;">
        ${overLimit ? 'Şimdi PRO Aboneliği Başlat (1.000₺/ay)' : 'PRO\'ya geçer misin?'}
      </a>
    </div>
    <p style="margin:16px 0 0;font-size:11px;color:#94a0b0;line-height:1.5;">
      İlk 100 promosyonuna katıldığın için teşekkürler! Yorumların ve önerilerin
      için ulaşabileceğin yer: destek@petstockpro.com
    </p>
  `);
...
${overLimit
    ? `⚠ ${input.productCount} ürünün FREE limitinin (50) üzerinde. Mevcut ürünlerin korunmaktadır ama yeni ürün ekleyemezsin. Devam etmek için PRO aboneliği başlat.`
    : `${input.productCount} ürününle FREE plan'da rahat çalışmaya devam edebilirsin.`}

PRO Aboneliği: ${input.upgradeUrl}

—
İlk 100 promosyonuna katıldığın için teşekkürler!
© 2026 PetStockPro`;

  return {
    subject: 'PetStockPro · PRO promosyonun sona erdi',
    htmlContent: html,
    textContent: text,
  };
}
